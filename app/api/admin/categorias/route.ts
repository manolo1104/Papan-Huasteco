...
export async function POST(req: Request) {
  ...
  const { data: row, error: dbErr } = await sb
    ...
    .single();
  if (dbErr) {
    const msg = dbErr.code === "23505" ? "Ya existe una categoría con ese nombre." : dbErr.message;
    return NextResponse.json({ error: msg }, { status: 400 });
  }
  return NextResponse.json({ ok: true, categoria: row });
}

// ── Editar categoría (renombrar / activar / desactivar) ───────
...
export async function DELETE(req: Request) {
  ...
  if (!id) return NextResponse.json({ error: "Falta la categoría." }, { status: 400 });
  const { error } = await sb.from("caja_categorias").delete().eq("id", id);
